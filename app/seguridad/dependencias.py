...
def requerir_password_admin(
    request: Request, admin_password: str | None = Header(None, alias="X-Admin-Password")
):
    """
    ...
    if not admin_password or not hmac.compare_digest(
        admin_password.encode("utf-8"), password_esperada.encode("utf-8")
    ):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
