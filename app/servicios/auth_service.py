...
class AuthService:
    ...
    def __init__(
        ...
        security_detection_service: SecurityDetectionService | None = None,
    ):
        """
        ...
        if self.token_blacklist_repo.is_blacklisted(jti):
            ...
            if self.security_detection_service and user_id:
                self.security_detection_service.detect_token_reuse(
                    jti=jti, user_id=user_id, ip_address=None, user_agent=None
                )
    ...
    def logout(self, refresh_token: str, user_id: int, ip_address: str, user_agent: str) -> None:
        """
        ...
        self.token_blacklist_repo.add_to_blacklist(
            jti=jti, token_type="refresh", user_id=user_id, expires_at=expires_at, reason="logout"
        )
    ...
    def forgot_password(self, email: str) -> str | None:
        ...
        reset_token = PasswordResetToken(
            user_id=user.id, token=token_hash, expires_at=expires_at, used=False
        )
