...
def archive_old_logs(retention_days: int = None, delete_after_archive: bool = False):
    ...
    print(f"[{datetime.now(UTC)}] Iniciando archival de logs de auditoría...")
    ...
    try:
        ...
        print(f"[{datetime.now(UTC)}] Archival completado.")
        return len(old_logs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--delete", action="store_true", help="Eliminar logs de la BD después de archivar"
    )
    ...
    try:
        archived = archive_old_logs(
            retention_days=args.retention_days, delete_after_archive=args.delete
        )
        sys.exit(0)
    except Exception as e:
