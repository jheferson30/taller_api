...
from app.modelos import User, Role, AuditLog, ConfiguracionSeguridad
    ...
    def print_report(self):
        ...
        if self.errors:
            ...
            for error in self.errors:
                print(f"  ✗ {error.get('clave', 'N/A')}: {error.get('error', error.get('reason', 'Unknown'))}")
    ...
    def save_to_file(self, filename: str = "migration_report.txt"):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            if self.errors:
                ...
                f.write("─"*70 + "\n")
                for error in self.errors:
                    f.write(f"  ✗ {error.get('clave', 'N/A')}: {error.get('error', error.get('reason', 'Unknown'))}\n")
                f.write("\n")
