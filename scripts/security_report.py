...
def generate_security_report(days: int = 7, output_file: str = None):
    ...
    """
    print(f"[{datetime.now(UTC)}] Generando reporte de seguridad...")
    ...
    try:
        ...
        report_lines.append(
            f"REPORTE DE SEGURIDAD - {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        report_lines.append(
            f"Período: {cutoff_date.strftime('%Y-%m-%d')} a {datetime.now(UTC).strftime('%Y-%m-%d')}"
        )
        report_lines.append("=" * 80)
        report_lines.append("")
        ...
        failed_logins = (
            ...
            .filter(AuditLog.action == "LOGIN_FAILED", AuditLog.created_at >= cutoff_date)
        ...
        security_alerts = (
            db.query(AuditLog)
            .filter(AuditLog.action == "SECURITY_ALERT", AuditLog.created_at >= cutoff_date)
        ...
        if security_alerts:
            for alert in security_alerts:
                report_lines.append(
                    f"[{alert.created_at}] {alert.details.get('alert_type', 'UNKNOWN')}"
                )
        ...
        expired_count = (
            db.query(func.count(TokenBlacklist.id))
            .filter(TokenBlacklist.expires_at < datetime.now(UTC))
            .scalar()
        )
        ...
        reset_requests = (
            ...
            .filter(AuditLog.action == "PASSWORD_RESET", AuditLog.created_at >= cutoff_date)
            .scalar()
        )
        ...
        total_events = (
            db.query(func.count(AuditLog.id)).filter(AuditLog.created_at >= cutoff_date).scalar()
        )
        ...
        print(f"[{datetime.now(UTC)}] Reporte generado exitosamente.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--days", type=int, default=7, help="Número de días a incluir en el reporte (default: 7)"
    )
    parser.add_argument("--output", type=str, help="Archivo de salida (default: stdout)")
