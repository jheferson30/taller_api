...
def check_security_alerts(hours: int = 24, email: str = None):
    ...
    """
    print(f"[{datetime.now(UTC)}] Verificando alertas de seguridad...")
    ...
    try:
        ...
        alerts = (
            db.query(AuditLog)
            .filter(AuditLog.action == "SECURITY_ALERT", AuditLog.created_at >= cutoff)
        ...
        print(f"\n[{datetime.now(UTC)}] Verificación completada.")
        return len(alerts)
...
def send_email_notification(alerts, recipient_email: str):
    ...
    from app.configuracion.config import SMTP_FROM, SMTP_HOST, SMTP_PASSWORD, SMTP_PORT, SMTP_USER
    ...
    message_lines.append(
        "  SELECT * FROM audit_log WHERE action = 'SECURITY_ALERT' ORDER BY created_at DESC;"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hours", type=int, default=24, help="Número de horas a revisar (default: 24)"
    )
    parser.add_argument("--email", type=str, help="Email para enviar notificaciones (opcional)")
