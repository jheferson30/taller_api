...
class TestEndpointProtection:
    ...
    def test_protected_endpoint_rejects_blacklisted_token(self, client, db_session, test_user):
        ...
        blacklist_repo.add_to_blacklist(
            jti=jti, user_id=test_user.id, expires_at=datetime.now(UTC) + timedelta(minutes=15)
        )
        db_session.commit()
    ...
    def test_protected_endpoint_accepts_valid_token(self, client, db_session, test_user):
    ...
    def test_malformed_authorization_header(self, client):
        ...
        response = client.get("/users", headers={"Authorization": "Bearer "})
        assert response.status_code == 401
