...
class TestProperty1_PasswordHashingProducesVerifiableHashes:
    ...
    **Validates: Requirements 1.1**
    ...
    def test_hashed_password_is_verifiable(self, password):
        ...
        ), "Password hash should be verifiable with original password"
    ...
    def test_hash_rejects_incorrect_passwords(self, password, wrong_password):
        ...
        ), "Hash should reject incorrect password"
    ...
    def test_hash_format_is_consistent(self, password):
        ...
        assert (
            "$04$" in password_hash or "$4$" in password_hash
        ), "Hash should include cost factor $04$ or $4$"
...
class TestProperty2_UniqueSaltGeneration:
    ...
    **Validates: Requirements 1.2**
    ...
    def test_same_password_produces_different_hashes(self, password):
        ...
        assert (
            hash1 != hash2
        ), "Hashing the same password twice should produce different hashes due to unique salt"
    ...
    @settings(max_examples=50, suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(password1=valid_password(), password2=valid_password())
    def test_different_passwords_produce_different_hashes(self, password1, password2):
        ...
        assert password_hasher.verify_password(
            password1, hash1
        ), "Hash1 should verify with password1"
        assert not password_hasher.verify_password(
            password1, hash2
        ), "Hash2 should not verify with password1"
        assert password_hasher.verify_password(
            password2, hash2
        ), "Hash2 should verify with password2"
        assert not password_hasher.verify_password(
            password2, hash1
        ), "Hash1 should not verify with password2"
