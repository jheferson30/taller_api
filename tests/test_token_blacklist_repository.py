...
def test_add_to_blacklist(token_blacklist_repo, db):
    ...
    result_expires = (
        result.expires_at.replace(tzinfo=UTC)
...
def test_jti_uniqueness(token_blacklist_repo, db):
    ...
    token_blacklist_repo.add_to_blacklist(
        jti=jti, token_type="refresh", user_id=1, expires_at=expires_at, reason="logout"
    )
...
def test_blacklisted_at_is_set_automatically(token_blacklist_repo, db):
    ...
    ts = (
        result.blacklisted_at
        if result.blacklisted_at.tzinfo
        else result.blacklisted_at.replace(tzinfo=UTC)
    )
