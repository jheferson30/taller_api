...
def test_get_by_date_range(audit_repo, db):
    ...
    db.execute(
        text(f"UPDATE audit_log SET timestamp = '{(now - timedelta(days=5)).isoformat()}' WHERE id = {old_log.id}")
    )
    db.commit()
...
def test_timestamp_is_set_automatically(audit_repo):
    ...
    ts = created.timestamp if created.timestamp.tzinfo else created.timestamp.replace(tzinfo=timezone.utc)
