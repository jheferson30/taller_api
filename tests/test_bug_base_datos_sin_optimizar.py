...
class TestBaseDatosSinOptimizar:
    ...
    def test_latencia_consulta_tickets_filtrados_mayor_500ms(self, db_session):
        ...
        print(f"\n{'='*80}")
        print(f"MEDICIÓN DE LATENCIA - Consulta Filtrada")
        print(f"{'='*80}")
        ...
        if total_tickets >= 1000:
            ...
            print(f"⚠ Advertencia: Solo {total_tickets} tickets en BD. "
    ...
    def test_n_plus_one_queries_al_cargar_relaciones(self, db_session, db_engine):
        ...
        try:
            ...
            for ticket in tickets:
                ...
                procesos = db_session.query(TicketProceso)\
                    .filter(TicketProceso.ticket_id == ticket.id).all()
                repuestos = db_session.query(TicketRepuesto)\
                    .filter(TicketRepuesto.ticket_id == ticket.id).all()
                fotos = db_session.query(TicketFoto)\
                    .filter(TicketFoto.ticket_id == ticket.id).all()
            ...
            print(f"\n{'='*80}")
            print(f"CONTEO DE QUERIES - N+1 Problem")
            print(f"{'='*80}")
            ...
            print(f"✓ Eager loading implementado: 0 queries adicionales (comportamiento correcto)")
    ...
    def test_endpoint_tickets_sin_paginacion_retorna_todos(self, db_session):
        ...
        print(f"\n{'='*80}")
        print(f"VERIFICACIÓN DE PAGINACIÓN")
        print(f"{'='*80}")
        print(f"Total tickets en BD: {total_tickets}")
        ...
        if total_tickets > 50:
            ...
            print(f"✓ Paginación implementada: {len(tickets)} tickets <= 50 (comportamiento correcto)")
        else:
            print(f"⚠ Advertencia: Solo {total_tickets} tickets en BD. "
    ...
    def test_no_existe_indice_compuesto_estado_fecha(self, db_engine):
        ...
        print(f"\n{'='*80}")
        print(f"VERIFICACIÓN DE ÍNDICES - idx_tickets_estado_fecha")
        print(f"{'='*80}")
        ...
        assert len(indices) > 0, (
            f"Bug Condition: El índice idx_tickets_estado_fecha DEBE existir "
            f"para optimizar consultas por estado+fecha, pero no fue encontrado. "
            f"En código sin corregir, este índice no existe. "
            f"Si este test falla, confirma que el bug existe."
        )
        
        print(f"✓ Índice idx_tickets_estado_fecha existe (comportamiento correcto)")
    ...
    def test_property_consultas_sin_indices_son_lentas(self, estado, dias_atras, db_session):
        ...
        print(f"  ✓ Consulta optimizada con índices")
