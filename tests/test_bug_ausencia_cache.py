...
class TestAusenciaCache:
    ...
    def test_redis_no_esta_en_requirements(self):
        ...
        try:
            ...
            tiene_redis_explicito = any(
                l.startswith('redis==') or l.startswith('redis>=') or l == 'redis'
                for l in lineas
            )
    ...
    def test_endpoint_estadisticas_no_tiene_decorador_cache(self):
        ...
        """
        try:
            with open('app/rutas/economia_ruta.py', 'r', encoding='utf-8') as f:
                contenido = f.read()
    ...
    @pytest.mark.skipif(
        ...
        reason="API tests deshabilitados"
    )
        ...
        try:
            ...
            for i in range(10):
                ...
                response = requests.get(
                    f"{api_base_url}/economia/estadisticas",
                    headers=headers,
                    timeout=10
                )
                fin = time.time()
    ...
    @given(
        num_peticiones=st.integers(min_value=5, max_value=10)
    )
    @settings(
        max_examples=2,
