...
def valid_secret_key(draw):
    ...
    return draw(
        st.text(
            alphabet=st.characters(
                blacklist_categories=("Cs", "Cc"), min_codepoint=33, max_codepoint=126
            ),
            min_size=32,
            max_size=64,
        )
...
class TestProperty5_JWTTokenVerificationRoundTrip:
    ...
    **Validates: Requirements 1.5**
    ...
    def test_access_token_round_trip_verification(self, user, secret_key):
        ...
        assert payload["user_id"] == user.id, "Payload debe contener user_id correcto"
        assert payload["username"] == user.username, "Payload debe contener username correcto"

        # Verificar que los roles están presentes
        expected_roles = [role.name for role in user.roles]
        assert payload["roles"] == expected_roles, "Payload debe contener roles correctos"
    ...
    def test_refresh_token_round_trip_verification(self, user, secret_key):
        ...
        # Verificar que el payload contiene los datos correctos
        assert payload["user_id"] == user.id, "Payload debe contener user_id correcto"
        assert payload["token_type"] == "refresh", "Payload debe indicar token_type='refresh'"

    @settings(max_examples=50, suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(user=valid_user(), secret_key=valid_secret_key(), wrong_secret_key=valid_secret_key())
    ...
    **Validates: Requirements 1.6**
    ...
    def test_access_token_contains_all_required_fields(self, user, secret_key):
        ...
        for field in required_fields:
            assert field in payload, f"Access token payload debe contener campo '{field}'"

        # Verificar tipos de datos
        assert isinstance(payload["user_id"], int), "user_id debe ser un entero"
        assert isinstance(payload["username"], str), "username debe ser un string"
        assert isinstance(payload["roles"], list), "roles debe ser una lista"
        assert isinstance(payload["exp"], (int, float)), "exp debe ser un timestamp numérico"
        assert isinstance(payload["iat"], (int, float)), "iat debe ser un timestamp numérico"
        assert isinstance(payload["jti"], str), "jti debe ser un string (UUID)"
    ...
    def test_refresh_token_contains_required_fields(self, user, secret_key):
        ...
        for field in required_fields:
            assert field in payload, f"Refresh token payload debe contener campo '{field}'"
        ...
        assert isinstance(payload["exp"], (int, float)), "exp debe ser un timestamp numérico"
        assert isinstance(payload["iat"], (int, float)), "iat debe ser un timestamp numérico"
        assert payload["token_type"] == "refresh", "token_type debe ser 'refresh'"
...
class TestProperty48_UniqueJWTID:
    ...
    **Validates: Requirements 20.6**
    ...
    def test_access_and_refresh_tokens_have_different_jti(self, user, secret_key):
        ...
        assert access_jti != refresh_jti, "Access token y refresh token deben tener JTI diferentes"
