...
class TestProperty7_TokenValidationRejectsInvalidTokens:
    ...
    **Validates: Requirements 1.7**
    ...
    @given(username=valid_username(), email=valid_email(), password=valid_password())
    ...
    def test_expired_tokens_are_rejected(
        self, db_session, user_repo, password_hasher, username, email, password
    ):
        """
...
class TestProperty10_PasswordMigrationLogging:
    ...
    **Validates: Requirements 2.5**
    ...
    def test_password_migration_creates_audit_log(
        ...
        logs_before = (
            db_session.query(AuditLog).filter(AuditLog.action == "PASSWORD_MIGRATED").count()
        )
        ...
        migration_log = (
            ...
            .filter(AuditLog.action == "PASSWORD_MIGRATED", AuditLog.user_id == user.id)
...
class TestProperty12_GenericAuthenticationErrorMessages:
    ...
    **Validates: Requirements 6.1**
    ...
    def test_nonexistent_user_returns_generic_error(
        self, auth_service, nonexistent_username, password, ip_address, user_agent
    ):
        """
...
class TestProperty14_FailedLoginAttemptsAreAudited:
    ...
    **Validates: Requirements 6.4**
    ...
    def test_failed_login_creates_audit_log(
        ...
        logs_before = db_session.query(AuditLog).filter(AuditLog.action == "LOGIN_FAILED").count()
        ...
        logs_after = db_session.query(AuditLog).filter(AuditLog.action == "LOGIN_FAILED").count()
    ...
    def test_nonexistent_user_login_creates_audit_log(
        self, db_session, auth_service, nonexistent_username, password, ip_address, user_agent
    ):
        """
...
class TestProperty45_LogoutBlacklistsRefreshToken:
    ...
    **Validates: Requirements 20.2**
    ...
    def test_logout_adds_jti_to_blacklist(
        ...
        assert token_blacklist_repo.is_blacklisted(
            jti
        ...
        blacklist_entry = db_session.query(TokenBlacklist).filter(TokenBlacklist.jti == jti).first()
