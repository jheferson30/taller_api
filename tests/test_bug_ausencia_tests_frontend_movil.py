...
class TestAusenciaTestsFrontendMovil:
    ...
    def test_no_existe_carpeta_e2e(self):
        ...
        assert not e2e_dir.exists(), (
            f"Bug Condition: NO debería existir carpeta e2e/ en código sin corregir. "
            f"Si este test falla, los tests E2E ya fueron implementados."
        )
    ...
    @given(
        proyecto=st.sampled_from(['frontend', 'mobile_app'])
    )
    @settings(
    ...
    def test_property_proyectos_no_tienen_tests(self, proyecto):
        ...
        test_patterns = ['*.test.js', '*.test.jsx', '*.test.ts', '*.test.tsx', '*.spec.js', '*.spec.jsx']
        test_files = []
