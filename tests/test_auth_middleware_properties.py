...
class TestProperty11_ProtectedEndpointsRequireAuthentication:
    ...
    @given(
        username=valid_username(),
        email=valid_email(),
        invalid_token=st.text(alphabet=st.characters(min_codepoint=33, max_codepoint=126), min_size=10, max_size=100)
    )
...
class TestProperty47_BlacklistVerificationInTokenValidation:
    ...
    @given(
        username=valid_username(),
        email=valid_email(),
        blacklist_reason=st.sampled_from(["logout", "user_deactivated", "security_breach", "manual_revoke"])
    )
