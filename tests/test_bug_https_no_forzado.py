...
class TestHTTPSNoForzado:
    ...
    def test_http_no_redirige_a_https(self, api_base_url):
        ...
        try:
            ...
            response = requests.get(f"{api_base_url}/info", allow_redirects=False, timeout=5)
            ...
            print("  No hay redirección automática a HTTPS")
    ...
    def test_codigo_fuente_cookies_sin_secure_flag(self):
        ...
        """
        try:
            with open("app/rutas/auth_ruta.py", encoding="utf-8") as f:
                contenido = f.read()
    ...
    def test_samesite_es_lax_no_strict(self):
        ...
        try:
            ...
            tiene_samesite_lax = (
                'samesite="lax"' in contenido.lower() or "samesite='lax'" in contenido.lower()
            )
            tiene_samesite_strict = (
                'samesite="strict"' in contenido.lower() or "samesite='strict'" in contenido.lower()
            )
    ...
    @given(endpoint=st.sampled_from(["/info", "/auth/login", "/tickets", "/"]))
    ...
    def test_property_http_aceptado_en_todos_endpoints(self, endpoint, api_base_url):
        ...
        """
        try:
            response = requests.get(f"{api_base_url}{endpoint}", allow_redirects=False, timeout=5)
