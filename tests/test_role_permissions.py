...
class TestRolePermissions:
    ...
    def test_user_without_admin_role_cannot_access_users_endpoint(self, client, db_session):
        """Test usuario sin rol ADMIN no puede acceder a /users (403)"""
        token_manager = TokenManager()

        # Crear usuario sin rol ADMIN
        user = User(
            username="mecanico1", email="mecanico@test.com", password_hash="dummy", is_active=True
        )
        db_session.add(user)
        db_session.flush()
        ...
        access_token = token_manager.generate_access_token(
            user_id=user.id, username=user.username, roles=["MECANICO"]
        )
    ...
    def test_user_with_admin_role_can_access_users_endpoint(self, client, db_session, test_user):
    ...
    def test_user_can_access_own_profile_without_admin(self, client, db_session):
        """Test usuario puede ver su propio perfil sin ser ADMIN"""
        token_manager = TokenManager()

        # Crear usuario sin rol ADMIN
        user = User(
            username="mecanico2", email="mecanico2@test.com", password_hash="dummy", is_active=True
        )
        db_session.add(user)
        db_session.flush()
    ...
    def test_user_cannot_access_other_user_profile_without_admin(
        ...
        # Crear usuario sin rol ADMIN
        user = User(
            username="mecanico3", email="mecanico3@test.com", password_hash="dummy", is_active=True
        )
        db_session.add(user)
        db_session.flush()
    ...
    def test_multiple_roles_grant_combined_permissions(self, client, db_session):
        ...
        access_token = token_manager.generate_access_token(
            user_id=user.id, username=user.username, roles=["ADMIN", "MECANICO"]
        )
